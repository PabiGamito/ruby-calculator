...
#Want it to be able to get an input recognise the equation and solve it or try and solve it step by step.
equation=method.gets

#Defines the array that contains all the possible variables.
variables=[x, y, m, a]
...
def divide(num1, num2) #THIS DIVIDES ONE NUMBER BY THE OTHER
  puts " "
  puts "#{num1} / #{num2} = #{num1.to_f / num2.to_f}" rescue false 
end
...
def operator(num1) #OPERATION METHOD (Calls all pervious methods ^~^)
  ...
  operator = ""
  while operator == "" do
    while (operator != "+") && (operator != "|+|") && (operator != "-") && (operator != "|-|") && (operator != "*") && (operator != "|*|") && (operator != "/") && (operator != "|/|") do
